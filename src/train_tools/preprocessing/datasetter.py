...
from torchvision.utils import save_image  # for saving torch tensors as images
...
def data_distributer(root, dataset_name, batch_size, n_clients, partition, oracle_size=0, oracle_batch_size=None,):
    ...
    for client_idx, dataidxs in net_dataidx_map.items():
        ...
        local_loaders[client_idx]["train"] = DATA_LOADERS[dataset_name](root, train=True, batch_size=batch_size, dataidxs=dataidxs,)
    '''
    
    for client_idx, dataidxs in net_dataidx_map.items():
    
        # Assign local dataset and size
        local_loaders[client_idx]["datasize"] = len(dataidxs)
        local_loaders[client_idx]["train"] = DATA_LOADERS[dataset_name](root, train=True, batch_size=batch_size, dataidxs=dataidxs,)
    ...
    if net_dataidx_map_test is not None:
        ...
        for client_idx, dataidxs in net_dataidx_map_test.items():
            ...
            local_testloader = DATA_LOADERS[dataset_name](root, train=False, batch_size=batch_size, dataidxs=dataidxs,)
            local_loaders[client_idx]["test"] = local_testloader
            local_loaders[client_idx]["dist"] = get_dist_vec(local_testloader, num_classes)
...
def sharding_partition(all_targets, n_clients, shard_per_user, rand_set_all=[]):
    ...
    # Store each client's indices in "client_indices" directory
    save_dir = "client_indices"
    os.makedirs(save_dir, exist_ok=True)

    #save_dir = '/home/khotso/FedSync/baselines/train_tools/preprocessing'

    for client_id, indices in net_dataidx_map.items():
        file_path = os.path.join(save_dir, f"client_{client_id}_indices.npy")
